...
public class Table3D extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        // Create a ColorPicker to change the color of the table and legs
        ColorPicker colorPicker = new ColorPicker();
        colorPicker.setValue(Color.BROWN); // Default color

        // Update material color when the user selects a color
        colorPicker.setOnAction(event -> {
            Color selectedColor = colorPicker.getValue();
            updateTableColor(seat, selectedColor);
            updateLegColor(leg1, selectedColor);
            updateLegColor(leg2, selectedColor);
            updateLegColor(leg3, selectedColor);
            updateLegColor(leg4, selectedColor);
        });

        // Create a VBox to hold the color picker and add it to the scene
    ...
    private void updateTableColor(Box table, Color color) {
        PhongMaterial material = new PhongMaterial();
        material.setDiffuseColor(color);
        table.setMaterial(material);
    }

    private void updateLegColor(Box leg, Color color) {
        PhongMaterial material = new PhongMaterial();
        material.setDiffuseColor(color);
        leg.setMaterial(material);
    }
